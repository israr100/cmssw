 ...
 */
  ...
  while (!olddata.eof() ) { 
    ...
    //count++;
    //if (count > MAX_SIZE + 3) exit(1);
  }
  olddata.close();
  
  std::ifstream newdata;
  std::ofstream myPedsFile("diff_GoodVals_pedSummary_Feb_09_2024.dat",std::ios::out);
  newdata.open("GoodVals_pedSummary_Feb_09_2024.dat",std::ios::in); 
  if(!newdata) {
    std::cerr <<"Error: GoodVals_pedSummary_Feb_09_2024.dat -> no such file!"<< std::endl;
    exit(1);
  }
  ...
  for(int i=0; i<MAX_SIZE;++i){
    if (i%10000 == 0) std::cout<<"Comparing PEDESTALS, please wait! "<< i << std::endl;
    for (unsigned int k=0;k<new_index_id.size()-1;++k){
      if(old_index_id[i] == new_index_id[k]){
	diffPeds[k]=(new_peds[k] - old_peds[i]/10.)/(old_peds[i]/10.);
  ...
  std::ofstream myXtalkFile("diff_GoodVals_xtalkSummary_Feb_09_2024.dat",std::ios::out);

  newdata1.open("GoodVals_xtalkSummary_Feb_09_2024.dat",std::ios::in); 
  if(!newdata1) {
    std::cerr <<"Error: GoodVals_xtalkSummary_Feb_09_2024.dat  -> no such file!"<< std::endl;
    exit(1);
  }
  ...
  for(int i=0; i<MAX_SIZE;++i){
    ...
    if (i%10000 == 0) std::cout<<"Comparing CROSSTALK, please wait! "<< i << std::endl;
    for (unsigned int k=0;k<new_index_id.size()-1;k++){
      counter1=new_index_id[k];
      if(counter == counter1){

	if ((abs(old_Rxtalk[i])/10000000.) > .000001) diffXtalkR[k]= (new_Rxtalk[k] -  old_Rxtalk[i]/10000000.)/(old_Rxtalk[i]/10000000.);
	if ((abs(old_Lxtalk[i])/10000000.) > .000001) diffXtalkL[k]= (new_Lxtalk[k] - old_Lxtalk[i]/10000000.)/(old_Lxtalk[i]/10000000.) ;
	if ((abs(old_Rint[i])/100000.) > .000001) diffIntR[k]=  (new_Rint[k] -  old_Rint[i]/100000.)/(old_Rint[i]/100000.);
	if ((abs(old_Lint[i])/100000.) > .000001) diffIntL[k]=  (new_Lint[k] - old_Lint[i]/100000.)/(old_Lint[i]/100000.) ;
  ...
  std::ofstream myGainsFile("diff_GoodVals_gainSummary_Feb_09_2024.dat",std::ios::out);
  newdata2.open("GoodVals_gainSummary_Feb_09_2024.dat",std::ios::in); 
  if(!newdata2) {
    std::cerr <<"Error: GoodVals_gainSummary_Feb_09_2024.dat -> no such file!"<< std::endl;
    exit(1);
  }
  ...
  for(int i=0; i<MAX_SIZE;++i){
    ...
    if (i%10000 == 0) std::cout<<"Comparing GAINS, please wait! "<< i << std::endl;    
 
    for (unsigned int k=0;k<new_index_id.size()-1;k++){
      counter1=new_index_id[k];
      if(counter == counter1){
	diffGains[k]= (new_gains[k] - old_gains[i]/1000.)/(old_gains[i]/1000.);
  ...
  std::ofstream myMatrixFile("diff_GoodVals_matrixSummary_Feb_09_2024.dat",std::ios::out);
  newdata3.open("GoodVals_matrixSummary_Feb_09_2024.dat",std::ios::in); 
  if(!newdata3) {
    std::cerr <<"Error: GoodVals_matrixSummary_Feb_09_2024.dat -> no such file!"<< std::endl;
    exit(1);
  }
  ...
  for(int i=0; i<MAX_SIZE;++i){
    if (i%10000 == 0) std::cout<<"Comparing NOISE MATRIX, please wait! "<< i << std::endl;
	...
	if (abs( old_el33[i]/1000. ) > .00000001)	  diff_el33[k] = ( new_el33[k] - old_el33[i]/1000. )/( old_el33[i]/1000. ) ;
	if (abs( old_el34[i]/1000. ) > .00000001)	  diff_el34[k] = ( new_el34[k] - old_el34[i]/1000. )/( old_el34[i]/1000. ) ;
	if (abs( old_el35[i]/1000. ) > .00000001)	  diff_el35[k] = ( new_el35[k] - old_el35[i]/1000. )/( old_el35[i]/1000. ) ;
	if (abs( old_el44[i]/1000. ) > .00000001)	  diff_el44[k] = ( new_el44[k] - old_el44[i]/1000. )/( old_el44[i]/1000. ) ;
	if (abs( old_el45[i]/1000. ) > .00000001)	  diff_el45[k] = ( new_el45[k] - old_el45[i]/1000. )/( old_el45[i]/1000. ) ;
	if (abs( old_el46[i]/1000. ) > .00000001)         diff_el46[k] = ( new_el46[k] - old_el46[i]/1000. )/( old_el46[i]/1000. ) ;
	if (abs( old_el55[i]/1000. ) > .00000001)	  diff_el55[k] = ( new_el55[k] - old_el55[i]/1000. )/( old_el55[i]/1000. ) ;
	if (abs( old_el56[i]/1000. ) > .00000001)	  diff_el56[k] = ( new_el56[k] - old_el56[i]/1000. )/( old_el56[i]/1000. ) ;
	if (abs( old_el57[i]/1000. ) > .00000001)	  diff_el57[k] = ( new_el57[k] - old_el57[i]/1000. )/( old_el57[i]/1000. ) ;
	if (abs( old_el66[i]/1000. ) > .00000001)	  diff_el66[k] = ( new_el66[k] - old_el66[i]/1000. )/( old_el66[i]/1000. ) ;
	if (abs( old_el67[i]/1000. ) > .00000001)	  diff_el67[k] = ( new_el67[k] - old_el67[i]/1000. )/( old_el67[i]/1000. ) ;
	if (abs( old_el77[i]/1000. ) > .00000001)	  diff_el77[k] = ( new_el77[k] - old_el77[i]/1000. )/( old_el77[i]/1000. ) ;
